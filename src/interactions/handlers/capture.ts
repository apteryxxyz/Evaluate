...
export async function handleCapturing(
  ...
  const startedAt = Number(process.env.START_TIMESTAMP);
  ...
  if (isBackup) messagePayload.content = t.capture.result.backup_service();
  if (!imageBuffer) messagePayload.content = t.capture.result.service_down();
